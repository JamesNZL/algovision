...
I also needed to design a simple [[Current Amplifier]] stage for the common-anode input of each LED, as the shift register can only source $\approx 7\,\text{mA}$ per output.

Lastly, I needed to determine the necessary [[VLED Bypassing]] [[Capacitors]] to produce a 'well-regulated LED supply voltage ($V_\text{LED}$). 
